...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        if latestPacket["rgb"] is not None:
            ...
            cv2.putText(frameRgb, str(rgb_fps)+' '+str(camRgb.getVideoSize()), (7, 70), font, 1, (100, 255, 0), 1, cv2.LINE_AA)
        ...
        if latestPacket["depth"] is not None:
            ...
            if (time.time() - start_time2) >= 1 :
                depth_fps=depth_counter
                depth_counter = 0
                start_time2 = time.time()
            cv2.putText(frameDepth, str(depth_fps)+' '+str(camRgb.getVideoSize()), (7, 70), font, 1, (100, 255, 0), 1, cv2.LINE_AA)
        ...
        if frameRgb is not None and frameDepth is not None:
            ...
            if (time.time() - start_time3) >= 1 :
                rgb_depth_fps=rgb_depth_counter
                rgb_depth_counter = 0
                start_time3 = time.time()
            cv2.putText(blended, str(rgb_depth_fps)+' '+str(camRgb.getVideoSize()), (7, 70), font, 1, (100, 255, 0), 1, cv2.LINE_AA)
